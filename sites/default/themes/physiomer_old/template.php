...
function physiomer2013_preprocess_node(&$vars){
    if ('product' == $vars['type']) {
        ...
        if (isset($vars['node'])) {
            ...
            drupal_add_css(
                '#node-'. $vars['node']->nid .'.node-product .product-btn { background-color: #'. physiomer2013_get_product_color($node) .' !important; }',
                array(
                    'group' => CSS_THEME,
                    'type' => 'inline',
                    'media' => 'screen',
 ...
 // krumo($vars['theme_hook_suggestions']);
 	// krumo($vars);
	// physiomer2013_replaceReadmoreLinkByReadmoreField($vars);
}
...
function physiomer2013_get_product_color($node) {
...
function physiomer2013_preprocess_block(&$vars){
// krumo($vars['theme_hook_suggestions']);
...
function physiomer2013_preprocess_page(&$vars){
    global $language;
    ...
    if ('search' == arg(0) && 'content' == arg(1)) {
        physiomer2013_format_search_results($vars);
    }
...
function physiomer2013_menu_local_tasks(&$vars) {
    $output = '';
...
function physiomer2013_preprocess_html(&$vars) {
    if ('node' == arg(0) && arg(1) > 0) {
        $node = node_load(arg(1));
        if ($node->type == 'home') {
...
function physiomer2013_process_html(&$vars) {
...
function physiomer2013_preprocess_field(&$vars, $hook){
 // krumo($vars['theme_hook_suggestions']);
    // krumo($vars);
...
function physiomer2013_breadcrumb($vars) {
...
function physiomer2013_preprocess_views_view(&$vars) {
    if ($vars['display_id'] == 'block_products_gamme') {
        // krumo($vars);
    }
}

function physiomer2013_preprocess_search_results(&$vars) {
}

function physiomer2013_menu_link__menu_block__menu_pharmacien(&$vars) {
    return physiomer2013_menu_link__menu_block__main_menu($vars);
}
function physiomer2013_menu_link__menu_block__menu_medecin(&$vars) {
    return physiomer2013_menu_link__menu_block__main_menu($vars);
}
function physiomer2013_menu_link__menu_block__main_menu(&$vars) {
...
function physiomer2013_menu_link__menu_block__menu_menu_footer(&$vars) {
    // krumo($vars);
    $element = $vars['element'];
    $sub_menu = '';
 ...
 * 
 * @see  physiomer2013_preprocess_node()
 * 
 * @param  array &$vars [description]
 * @return void
 */
function physiomer2013_replaceReadmoreLinkByReadmoreField(&$vars) {
	if (
		   isset($vars['content'])
...
// function physiomer2013_preprocess_file_entity(&$variables) {
 ...
 */
function physiomer2013_get_field_image_info($__node, $__fieldName, $__key = 0, $__info = 'all') {
...
function physiomer2013_format_search_results(&$vars) {
